...
public:
     ...
     */
    virtual pair<int, int> think(const Tablero& tablero) override;


    // --- Métodos de Juego  ---
    pair<int, int> JuegaAleatorio(const Tablero& tablero);
    pair<int, int> JuegaInteligente(const Tablero& tablero);
     ...
     */
    Resultado Status(const Tablero &tablero, pair<int,int> &Mov);

    /**
     * @brief Implementación del algoritmo MiniMax clásico.
     */
    double minimax(const Tablero &tablero, int profundidad, int prof_Max, pair<int,int> &Mov);

    /**
     * @brief Implementación del algoritmo MiniMax con Poda Alfa-Beta.
     */
    double alfaBeta(const Tablero &tablero, int profundidad, int prof_Max, double alfa, double beta, pair<int,int> &Mov);
...
private:
    ...
    atomic<bool> abortarBanda; // Flag para interrumpir la búsqueda
    pair<int, int> mejorMovimientoH; // Mejor movimiento encontrado en la última profundidad completada
